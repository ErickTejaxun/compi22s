package servidol.server.manejador;
...
import servidor.analizadores.Error;
import servidor.analizadores.Grafica;
import servidor.analizadores.Nodo;
import servidor.interpretes.Analizadores.USQL.analizador.ParserUSQL;
import servidor.interpretes.Analizadores.XML.analizador.ParseException;
import servidor.interpretes.Analizadores.XML.analizador.ParserXML;
...
public class Archivos implements Constantes{
    ...
    /*DIRECTORIOS*/
        //BASE
    private static final String rootDir = "/home/erick/" + dbms + "/";
        //SISTEMA DE ARCHIVOS
    ...
    public static Nodo levantarXML(String path){
        ...
        try{
            ...
            new Grafica().graficar(astXML, "/home/erick/Desktop/ASTs/xml/" + new File(path).getName());
            
        } catch (ParseException ex) {
            Consola.writeln("ERROR EN EL PARSEO DE ARCHIVO XML {" + new File(path).getName() + "}");
            Consola.writeln(ex.getLocalizedMessage());
        } catch(servidor.interpretes.Analizadores.XML.analizador.TokenMgrError ex){
    ...
    public static Nodo parsearUSQL(int codigo, String instruccion) throws IOException{
        ...
        try {
            ...
            new Grafica().graficar(astUSQL, "/home/erick/Desktop/ASTs/usql/instruccion");
            
        }catch(servidor.interpretes.Analizadores.USQL.analizador.TokenMgrError ex){
            Consola.writeln(ex.getLocalizedMessage());
            String token = "ERROR";
        ...
        }catch (servidor.interpretes.Analizadores.USQL.analizador.ParseException ex) {
            Consola.writeln(ex.getLocalizedMessage());
            String token = "ERROR";
            String valor = Error.lenguaje(
                    codigo, 
