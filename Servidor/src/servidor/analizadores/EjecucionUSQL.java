...
import servidol.server.manejador.Metodo;
import servidol.server.manejador.Objeto;
...
public class EjecucionUSQL {
    ...
    public static String ejecutar(Nodo usqlSent) throws IOException{
        ...
        switch(usqlSent.token){
                ...
                case "FUNC":
                    ...
                    Metodo met = Archivos.bbdd.getMetodo(Server.actualDB, idMet);
                    Consola.writeln(claseMet + " [" + idMet + "] creado exitosamente.\n");
                    EjecucionSSL.ejecutar.addmetodounico(met);
                    break;
    ...
    public static ArrayList<HashMap<String, String>> select(Nodo select){
        ...
        if(where != null){
            ...
            for(HashMap<String, Simbolo> filaTemporal : tablaTemporal){
                ...
                if(EjecucionSSL.ejecutar.Expre(cond).value.equals("1")){
                    ...
                    if(lcol.token.equals("*")){
                        ...
                        if(!tablaResultado.contains(filaResultado))
            ...
            if(where.hijos.size() == 2){
                ...
                if(orderMode.equals("ASC"))
                    Collections.sort(tablaResultado,new MapComparatorAsc(orderCol));
                else
                    Collections.sort(tablaResultado,new MapComparatorDesc(orderCol));
    ...
    private static String update(Nodo update){
        ...
        for(HashMap<String, Simbolo> filaTemporal : tablaTemporal){
            row = new Nodo("row");
            for(String colName : filaTemporal.keySet()){
                String[] columna = colName.split("\\.");
                Nodo cn = new Nodo(columna[1]);
        ...
        for(int i =0; i < tablas.size(); i++){
            ...
            for(Nodo row : actual.hijos){
                ...
                for(Nodo col : row.hijos){
                    ...
                    s.tipe = Archivos.bbdd.getTipoColumna(Server.actualDB, nombre, col.token);
                    copiaFila.put(nombre + "." + col.token, s);
                }
    ...
    private static String restaurar(Nodo nodo) throws IOException {
        String response = "";
        
        Nodo idFile = nodo.getHijo(0);
        String fileName = Archivos.bbddDir + Server.actualDB + "/" + idFile.valor;
        
        Nodo backupXML = Archivos.levantarXML(fileName);
        
        for(Nodo instr : backupXML.hijos){
            String sent = instr.getHijo(0).valor;
            Nodo astUSQL = Archivos.parsearUSQL(0, sent);
        
            if(astUSQL.token.equals("ERROR"))
                 response += astUSQL.valor;
            else{
                response += InterpreteUSQL.interpretarSentenciasUsql(0, astUSQL, sent);
            }
